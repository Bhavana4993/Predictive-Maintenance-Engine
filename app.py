...
with col_left:
    ...
    with m1:
        ...
        st.metric("Mechanical Power", f"{power_watts / 1000:.2f} kW", delta="Normal Range" if 3500 <= power_watts <= 9000 else "Outside Safe Power Window", delta_color="normal" if 3500 <= power_watts <= 9000 else "inverse")
    ...
    st.markdown(f"- **Cost-Optimized Failure Decision Threshold:** `{optimal_threshold if artifacts_loaded else 0.25:.2f}`")
    ...
    if artifacts_loaded:
        ...
        if is_failure_predicted:
            ...
            <div class="status-warning">
                🚨 WARNING: HIGH FAILURE RISK DETECTED!<br>
